...
func TestMergeComponentRef_AdvancedFields(t *testing.T) {
	...
	t.Run("installFile overlay takes precedence", func(t *testing.T) {
		base := ComponentRef{
			Name:        "nfd-operator",
		...
		overlay := ComponentRef{
			Name:        "nfd-operator",
			InstallFile: "components/nfd/olm/custom-install.yaml",
		}
		result := mergeComponentRef(base, overlay)
		if result.InstallFile != "components/nfd/olm/custom-install.yaml" {
			t.Errorf("installFile = %q, want %q", result.InstallFile, "components/nfd/olm/custom-install.yaml")
		}
	})

	t.Run("resourcesFile from overlay", func(t *testing.T) {
		base := ComponentRef{Name: "test", ResourcesFile: "base/resources/resources.yaml"}
		overlay := ComponentRef{Name: "test", ResourcesFile: "overlay/resources/resources.yaml"}
		result := mergeComponentRef(base, overlay)
		if result.ResourcesFile != "overlay/resources/resources.yaml" {
			t.Errorf("resourcesFile = %q, want overlay/resources/resources.yaml", result.ResourcesFile)
		}
	})

	t.Run("resourcesFile inherited from base", func(t *testing.T) {
		base := ComponentRef{Name: "test", ResourcesFile: "base/resources/resources.yaml"}
		overlay := ComponentRef{Name: "test"}
		result := mergeComponentRef(base, overlay)
		if result.ResourcesFile != "base/resources/resources.yaml" {
			t.Errorf("resourcesFile = %q, want base/resources/resources.yaml (inherited)", result.ResourcesFile)
		}
	})
...
func TestApplyComponentDefaults_OLM(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: ComponentRef{
				...
				ResourcesFile: "components/gpu-operator/olm/resources.yaml", // Default from auto-discovery
				Kinds:         []string{"ClusterPolicy"},
		...
		},
		{
			...
			expected: ComponentRef{
				...
				ResourcesFile: "components/nfd-operator/olm/resources.yaml",
				Kinds:         []string{"NodeFeatureDiscovery"},
				Source:        "",
				Version:       "",
				Chart:         "",
		...
		},
		{
			...
			expected: ComponentRef{
				Name:          "test-operator",
				Type:          ComponentTypeOLM,
				ResourcesFile: "components/test-operator/olm/resources.yaml",
				Kinds:         []string{"CustomResourceA", "CustomResourceB"},
				Source:        "",
				Version:       "",
				Chart:         "",
		...
		{
			name: "OLM component - apply installFile from config",
			ref: &ComponentRef{
				Name: "nfd-operator",
				Type: ComponentTypeOLM,
			},
			config: &ComponentConfig{
				...
				OLM: OLMConfig{
					DefaultNamespace: "openshift-nfd",
					InstallFile:      "components/nfd-operator/olm/install.yaml",
					Kinds:            []string{"NodeFeatureDiscovery"},
				},
			},
			expected: ComponentRef{
		...
		{
			name: "OLM component - preserve existing installFile",
			ref: &ComponentRef{
				Name:        "test-operator",
				Type:        ComponentTypeOLM,
				InstallFile: "custom/install.yaml",
			},
			config: &ComponentConfig{
				Name: "test-operator",
				OLM: OLMConfig{
					InstallFile: "default/install.yaml",
			...
			expected: ComponentRef{
				Name:          "test-operator",
				Type:          ComponentTypeOLM,
				InstallFile:   "custom/install.yaml", // Preserved
				ResourcesFile: "components/test-operator/olm/resources.yaml",
				Kinds:         []string{"TestResource"},
				Source:        "",
				Version:       "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check InstallFile
			if tt.ref.InstallFile != tt.expected.InstallFile {
				t.Errorf("InstallFile = %q, want %q", tt.ref.InstallFile, tt.expected.InstallFile)
			}

			// Check ResourcesFile
			if tt.ref.ResourcesFile != tt.expected.ResourcesFile {
				t.Errorf("ResourcesFile = %q, want %q", tt.ref.ResourcesFile, tt.expected.ResourcesFile)
			}
		})
	}
}
