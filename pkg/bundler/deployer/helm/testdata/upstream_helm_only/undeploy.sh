...
# ==============================================================================
# Generic reverse loop handles both Direct and Helm components:
# - Direct components (with uninstall.sh) use kubectl delete
# - Helm components (local-helm or upstream-helm) use helm uninstall
for dir in $(ls -1d "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/ 2>/dev/null | sort -r); do
  [[ -d "${dir}" ]] || continue
  base="${dir%/}"
  base="${base##*/}"
  ...
  # Check if this is a Direct component (has uninstall.sh)
  if [[ -f "${dir}/uninstall.sh" ]]; then
    # Direct component - use kubectl delete via uninstall.sh
    ( cd "${dir}" && bash uninstall.sh ) || echo "Warning: uninstall.sh failed for ${name}" >&2
    delete_orphaned_webhooks_for_ns "${ns}"
  else
    # Helm component - use helm uninstall
