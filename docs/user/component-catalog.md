...
| **k8s-nim-operator-ocp-olm** | OLM installer for the NVIDIA NIM Operator on OpenShift. Creates the OperatorGroup and Subscription resources that install the certified NIM Operator via the Operator Lifecycle Manager. Unlike the other OCP operators, has no paired CR-phase component — the NIM Operator's owned CRDs (NimCache, NimService, NimPipeline, etc.) are workload-scoped resources applied by consumers, not a bootstrap CR the platform overlay itself needs. OCP-specific. | [NVIDIA NIM Operator (Certified)](https://catalog.redhat.com/en/software/containers/nvidia/nim-operator-bundle/677ee00b53ccfc91f98e976c) |
